...
// Map-area clamping, waypoint pathfinding, and point-to-point movement kernel.
public partial class SmartNpcAI
{
    ...
    public int maxPathNodes = 320;
    public int maxPathSteps = 120;
    public float maxPathSearchDistance;
        ...
        NpcMapZone? targetZone = null)
    {
        ...
        NpcMapZone? previousMovementTargetZone =
            movementTargetZone;
        ...
        try
        {
            ...
            bool usingPathWaypoint =
                TryGetSmartPathWaypoint(
                    finalTarget,
                    out Vector3 pathWaypoint);
            ...
            else if (ShouldRequirePathForDirectMove(
                         finalTarget) &&
                ...
                IsMovementBlocked(direction))
            {
                ...
                else if (TryCommitObstacleScanTarget(
                             direction,
                             finalTarget))
                {
                    return;
                    ...
                    TryChooseObstacleDetourDirection(
                        direction,
                        finalTarget,
                        out Vector2 detourDirection))
                {
                    if (TryCommitObstacleAvoidTarget(
                            detourDirection))
                    {
                        return;
                    }
    ...
    void ClampInsideCurrentMapArea()
    {
        ...
        if (Vector2.Distance(clamped, transform.position) <=
            0.001f)
        {
            return;
        }
            ...
            movementTargetZone.Value == currentMapArea.zone)
        {
                ...
                nearestSameZone.areaBounds != null)
            {
                Vector3 nearestPoint =
                    nearestSameZone.ClosestPoint(position);
                if (Vector2.Distance(
                        nearestPoint,
                        position) <=
                    Mathf.Max(
    ...
    bool IsInsideCurrentMapArea(Vector2 position)
    {
        ...
        Vector2 closest =
            currentMapArea.areaBounds.ClosestPoint(position);
        return Vector2.Distance(closest, position) <=
            0.02f;
    }

    bool IsTeleportEntryTargetForCurrentArea(
        Vector3 position)
    {
        if (currentMapArea == null ||
            !movementTargetZone.HasValue ||
            movementTargetZone.Value ==
                currentMapArea.zone)
        {
            return false;
        }
        ...
        foreach (NpcTeleportGate gate in
                 NpcTeleportGate.Gates)
        {
            if (gate == null ||
    ...
    bool ShouldRequirePathForDirectMove(
        Vector3 finalTarget)
    {
    ...
    bool TryBuildSmartPath(Vector3 finalTarget)
    {
        ...
        if (pathCellSize <= 0.05f ||
            !IsInsideCurrentMapArea(finalTarget))
        {
            RecordSmartPathResult(
                false,
                visited,
                pathStartTime);
            return false;
        ...
        start = PathCellToWorld(startCell);
        finalTarget = PathCellToWorld(targetCell);

        bool hasRememberedPath =
            TryGetRememberedPathCandidate(
                start,
                finalTarget);
        ...
        float pathStartTime)
    {
        ...
        float baseDelay =
            Mathf.Max(2f, pathReplanCooldown);
        if (success)
        {
        ...
        consecutiveSmartPathFailures =
            Mathf.Min(
                consecutiveSmartPathFailures + 1,
                4);
        ...
        float failDelay =
            Mathf.Min(
                ...
                baseDelay *
                (1f + consecutiveSmartPathFailures));
        ...
        Vector3 finalTarget)
    {
        ...
        TrimPathStartForCurrentPosition(
            rememberedPathBuffer);
        ...
        List<Vector3> path)
    {
        ...
        score +=
            Vector2.Distance(previous, finalTarget);
        return score;
    }

    void TrimPathStartForCurrentPosition(
        List<Vector3> path)
    {
        if (path == null)
        {
        ...
        Vector2Int offset)
    {
        ...
        return IsPathCellWalkable(
                   from +
                   new Vector2Int(offset.x, 0)) &&
            IsPathCellWalkable(
                   from +
                   new Vector2Int(0, offset.y));
    }

    bool IsPathCellWalkable(Vector2Int cell)
    {
        Vector3 world = PathCellToWorld(cell);
