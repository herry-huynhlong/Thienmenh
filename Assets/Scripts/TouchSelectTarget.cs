...
public partial class TouchSelectTarget : MonoBehaviour
{
    ...
    [Header("Selection")]
    [Tooltip("Optional assist radius when the tap does not land on a collider exactly.")]
    public float selectionAssistRadius = 0.1f;
    ...
    string BuildMonsterInfo(MonsterAI monster)
    {
        ...
        builder.AppendLine(NpcText.Label("age") + ": " + GetTargetAge(monster.transform));
        builder.AppendLine(NpcText.Label("lifespan") + ": " + GetTargetLifespan(monster.transform));
    ...
    string GetTargetAge(Transform target)
    {
        ...
        if (monster != null &&
            monster.GetAge() > 0)
        {
            return monster.GetAge().ToString();
        }
        ...
        out int maxLifespan)
    {
        ...
        if (monster != null)
        {
            maxLifespan = Mathf.Max(1, monster.GetLifespan());
            currentLifespan = Mathf.Max(0, monster.GetAge());
            return true;
        }
